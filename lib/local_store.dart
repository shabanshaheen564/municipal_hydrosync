...
class LocalStore {
  ...
  static Map<String, dynamic>? readCache(String key) {
    ...
    try {
      ...
      if (value is Map && value['data'] != null) return {'saved_at': value['saved_at'], 'data': value['data']};
    } catch (_) {}
    return null;
  }

  static Future<void> writeCache(String key, dynamic value) async {
    await _cache.put(key, jsonEncode({'saved_at': DateTime.now().toIso8601String(), 'data': value}));
  }
  ...
  static String enqueue(String method, String endpoint, Map<String, dynamic>? body) {
    final id = '${DateTime.now().microsecondsSinceEpoch}-${_queue.length}';
    _queue.put(id, jsonEncode({'id': id, 'method': method, 'endpoint': endpoint, 'body': body, 'created_at': DateTime.now().toIso8601String(), 'attempts': 0, 'status': 'pending'}));
    return id;
  }
  static List<Map<String, dynamic>> queueItems() {
    ...
    for (final key in _queue.keys) {
      ...
      if (raw is String) { try { result.add(Map<String, dynamic>.from(jsonDecode(raw))); } catch (_) {} }
    }
  ...
  static Future<void> setLastSyncNow() async => _meta.put('last_sync_at', DateTime.now().toIso8601String());

  static Future<void> patchEndpoint(String user, String endpoint, Map<String, dynamic> patch) async {
    for (final key in _cache.keys.toList()) {
      ...
      if (data is Map && data['id'] != null && endpoint.endsWith('/${data['id']}')) {
        await writeCache(keyString, {...Map<String, dynamic>.from(data), ...patch});
      } else if (data is Map && data['data'] is List) {
        final list = (data['data'] as List).map((item) {
          if (item is Map && endpoint.endsWith('/${item['id']}')) return {...Map<String, dynamic>.from(item), ...patch};
          return item;
        }).toList();
        await writeCache(keyString, {...Map<String, dynamic>.from(data), 'data': list});
  ...
  static Future<void> addPendingRecord(String user, String endpoint, Map<String, dynamic> body, String queueId) async {
    final localId = -DateTime.now().microsecondsSinceEpoch;
    final record = {...body, 'id': localId, 'queued': true, 'local_pending': true, 'local_queue_id': queueId, 'created_at': DateTime.now().toIso8601String()};
    for (final key in _cache.keys.toList()) {
      final keyString = '$key';
      ...
      if (cached == null) continue;
      final data = cached['data'];
      if (data is Map && data['data'] is List) await writeCache(keyString, {...Map<String, dynamic>.from(data), 'data': [record, ...(data['data'] as List)]});
      else if (data is List) await writeCache(keyString, [record, ...data]);
    }
  }
}